...
class Car(BoxLayout):
    ...
    def display_submodels(self,data):
        ...
        try:
            ...
            self.ids.console_log.text=("Invalid make, model or year. Check spelling ")
        except IndexError:
            self.ids.console_log.text=(("*"*5)+"Model year data not avaliable. Try a different year."+("*"*5))
    ...
    def create_submodel_list(self,rawdata):
        ...
        try: 
            ...
            self.ids.console_log.text="Check your inputs. No Data found"
    ...
    def cartrim1_select(self,btn,carid_request,car_make,car_model,car_year):
        ...
        self.ids.car1select.focus=True
    ...
    def cartrim2_select(self,btn,carid_request,car_make,car_model,car_year):
        ...
        self.ids.car2select.focus=True

    def compare_cars(self,btn,horsepower_request,torque_request,add_car):
        try:
            #requesting again b/c cant get data from other functions for now...
            data1=self.carid_request(self.ids.car1make.text,self.ids.car1model.text,self.ids.car1year.text,key1)
            data2=self.carid_request(self.ids.car2make.text,self.ids.car2model.text,self.ids.car2year.text,key1)
            ...
            #create dictionaries for user selection
            submodel1_dict=self.display_submodels(data1)
            submodel2_dict=self.display_submodels(data2)
            '''
            x=0
            ...
            self.ids.console_log.text="Please input a valid trim selection before proceeding"


class CarCompare(App):
    def build(self):
        try:
            os.remove("submodel.db") #clear database upon opening for speed
        except FileNotFoundError:
            pass
        return Car()
